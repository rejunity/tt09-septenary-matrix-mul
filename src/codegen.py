...
septenery_mapping = [0, .5, 1, 2, -.5, -1, -2] # 7 possible states
senary_mapping_a  = [0, .5, 1, 2,      -1, -2] # 6 possible states
senary_mapping_b  = [0,     1, 2, -.5, -1, -2] # 6 possible states
quinary_mapping_a = [0,     1, 2,      -1, -2] # 5 possible states
quinary_mapping_b = [0, .5, 1,    -.5, -1    ] # 5 possible states
ternary_mapping   = [0,     1,         -1    ] # 3 possible states
...
def pack_676_weights(weights):
    packed = 0
    for w, n, mapping in zip(weights,
                            itertools.cycle([6,7,6]),
                            itertools.cycle([senary_mapping_a,septenery_mapping,senary_mapping_a])):
        packed = (packed * n) + find_closest_index(mapping, w)
    return packed

def unpack_676_weights(packed, number_of_weights=3, senary_mapping_1=senary_mapping_a, senary_mapping_2=senary_mapping_a):
    weights = []
    # Inverting the dictionary
    for _, n, mapping in zip(range(number_of_weights),
                            itertools.cycle(reversed([6,7,6])),
                            itertools.cycle([senary_mapping_1,septenery_mapping,senary_mapping_2])):
        weights.append(mapping[packed % n])
        packed //= n
    return weights[::-1]
...
# test 676
senary_1 = [-2,-1,   0,.5,1,2]
# senary_2 = [-2,-1,-5,0   ,1,2] # @TODO: fix balanced senary with a pair of alternating encodings
senary_2 = senary_1
assert find_max_size(all_combinations(septenery, 3), pack_676_weights) <= 255
assert find_max_size(all_combinations(septenery, 3), pack_676_weights) == 6*7*6
assert check_if_all_unique(all_combinations(septenery, 3), pack_676_weights) == True
assert check_if_all_match(all_combinations(quinary, 3), pack_676_weights, unpack_676_weights) == True
assert check_if_all_match(itertools.product(senary_1, septenery, senary_2), pack_676_weights, unpack_676_weights) == True
...
def generate_verilog_module_to_unpack_676_weights(reverse_weight_order = True):
    print(\
    ...
    unpack_weights = unpack_676_weights
...
generate_verilog_module_to_unpack_676_weights()
